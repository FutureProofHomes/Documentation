...
3. **Conversation Agent** - The text transcription is processed by a rules-based engine (or perhaps an A.I.) which executes your command and generates a text response.  
4. **Text-to-Speech** - The text response is converted that into a synthetic voice response that is played back through the speaker.

That's a voice pipeline.  It's the backbone of any voice assistant.  And each step in a voice pipeline can be modified and customized to fit your needs.  What wake word do you want?  What language are you speaking?  Do you want a standard voice response or to hear Arndold Schwarzengger speak back to you?  Do you want Home Assistant, Google or OpenAI to process and execute your command?  Follow the steps below to set up a voice pipeline for your Satellite1.

Please read all of the below.  Then we highly recommend **[upgrading to a magical fallback conversation agent](/voice-pipeline/#magical-fallback-conversation-agent)**.  Also, don't forget to **[expose entities to your Satellite1 voice assistant!](/voice-pipeline/#exposing-entities-to-your-voice-assistant)**
...
???+ abstract "Combine a standard conversation agent with an AI conversation agent' (It's like magic!)"
    ...
    [Read more this feature release](https://www.home-assistant.io/blog/2024/12/04/release-202412/#let-your-voice-assistant-fall-back-to-an-llm-based-agent){ .md-button .md-button--primary }
...
Congtatulations!  You've created your own voice pipeline for your Satellite1.
